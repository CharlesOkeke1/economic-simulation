...
public class EventTrigger {
    ...
    public static void trigger(String stateName,int count) {
        ...
        if (count > 1) {
            ...
            // Debt crisis
            if (state.getDebtToGdpRatio() > 0.35) {
            ...
            // Oil crash
            if (state.getGdpGrowth() < -0.03) {
                state.getEventRisk().increaseRisk(Events.OIL_CRASH, 0.10);
            }

            // Forex boom
            if (state.getStability() > 0.75 && state.getGdpGrowth() > 0.03) {
                state.getEventRisk().increaseRisk(Events.FOREX_BOOM, 0.09);
            }

            // Insurgency
            if (state.getInfrastructure() < 40 && state.getStability() < 0.45) {
                state.getEventRisk().increaseRisk(Events.INSURGENCY, 0.10);
            }

            // War (rare)
            if (state.getStability() < 0.35 && Math.random() > 0.8) {
                state.getEventRisk().increaseRisk(Events.WAR, 0.12);
            }

            // Pandemic (rare)
            if (state.getStability() < 0.5 && Math.random() > 0.9) {
        ...
        if (chosenEvent != null) {
            ...
            if (state.getName().equals(pState)) {
                ...
                playerEvent = pState + " is experiencing a "
                        + EnumToString.convert(chosenEvent, "_");
            }
